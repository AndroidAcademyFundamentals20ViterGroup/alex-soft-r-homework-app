...
import com.s0l.movies.ui.MainActivity
    ...
    suspend fun show(movie: MovieEntity) {
        ...
        val smallContentView =
            RemoteViews(context.packageName, R.layout.notification_small).apply {
                ...
                setTextViewText(
                    R.id.rating,
                    context.getString(R.string.rating_text, movie.vote_average.toString())
                )
            }
        ...
        val loadSmallImage = loadImage(smallContentView, R.id.movie_image, movie.poster_path)
        val loadBigImage = loadImage(bigContentView, R.id.movie_image, movie.poster_path)
